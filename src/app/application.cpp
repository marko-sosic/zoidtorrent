    ...
    , m_commandLineArgs(parseCommandLine(this->arguments()))
{
    ...
    setOrganizationDomain("zoidtorrent.org");
#if !defined(DISABLE_GUI)
    setDesktopFileName("org.zoidtorrent.zoidtorrent");
    ...
    Logger::instance()->addMessage(tr("zoidtorrent %1 started", "zoidtorrent v3.2.0alpha started").arg(QBT_VERSION));
...
void Application::runExternalProgram(const BitTorrent::Torrent *torrent) const
{
    ...
    // See: https://github.com/zoidtorrent/zoidtorrent/issues/10925
...
void Application::sendNotificationEmail(const BitTorrent::Torrent *torrent)
{
    ...
    const QString content = tr("Torrent name: %1").arg(torrent->name()) + '\n'
        ...
        + tr("Thank you for using zoidtorrent.") + '\n';
    ...
    smtp->sendMail(pref->getMailNotificationSender(),
                     ...
                     tr("[zoidtorrent] '%1' has finished downloading").arg(torrent->name()),
                     content);
}
...
int Application::exec(const QStringList &params)
{
    ...
    const QString mesg = QString::fromLatin1("\n******** %1 ********\n").arg(tr("Information"))
        + tr("To control zoidtorrent, access the Web UI at %1")
...
void Application::initializeTranslation()
{
    ...
    if (m_translator.load(QLatin1String(":/lang/zoidtorrent_") + localeStr))
